...
double S0(const int K, const int N)
{
	double s = 0;
	for (int i = K; i <= N; i++)
		s += (1.*sin(i) * 1*cos(i)) /( 1 + 1*pow(sin(i), 2));
	return s;
}
double S1(const int K, const int N, const int i)
{
	if (i > N)
		...
		return (1*sin(i) * 1*cos(i)) / (1 + 1*pow(sin(i), 2))+S1(K, N, i+1);
}
double S2(const int K, const int N, const int i)
{
	if (i < K)
		...
		return (1*sin(i) * 1*cos(i)) / (1 + 1*pow(sin(i), 2))+S2(K, N, i-1);
}
double S3(const int K, const int N, const int i, double t)
{
	t = t + (1*sin(i) * 1*cos(i)) /( 1 + 1*pow(sin(i), 2));
	if (i >= N)
		return t;
	else
		return S3(K, N, i + 1, t);
